...
class MPCoreMCP(_NeedsMPClient):
    ...
    def _aggregate_fetch_results_from_mpids(
        ...
        summary_docs = self.client.materials.summary.search(material_ids = mpids, fields=MaterialMetadata._summary_fields())
        similarity_docs = self.client.materials.similarity.search(material_ids = mpids, fields=["sim","material_id"])

        robo_desc_by_mpid = {
            doc["material_id"] : doc["description"]
            for doc in self.client.materials.robocrys.search_docs(
                material_ids = mpids,
                fields=["description","material_id"]
            )
        }
        ...
        sim_scores_by_mpid = {
            doc["material_id"] : ", ".join(
                ...
                for entry in sorted(
                    doc["sim"], key = lambda e : e["dissimilarity"]
                )[:10]
            )
        ...
        return [
            FetchResult(  # type: ignore[call-arg]
                ...
                metadata = MaterialMetadata.from_summary_data(
                    doc,
                    structurally_similar_materials = sim_scores_by_mpid.get(doc["material_id"])
                )
            )
    ...
    def _validate_identifiers(self, idxs : list[str], limit_one_per_chemsys : bool = False) -> list[str]:
        ...
        chemsys : set[str] = {
            "-".join(sorted(idx.split("-")))
            for idx in non_mp_ids
            if "-" in idx
        }
        formula : set[str] = {
            idx
        ...
        if chemsys:
            summ_docs += self.client.materials.summary.search(
                chemsys = list(chemsys), fields = ["material_id","energy_above_hull","chemsys"]
            )
        if formula:
            summ_docs += self.client.materials.summary.search(
                formula = list(formula), fields = ["material_id","energy_above_hull","chemsys"]
            )
        ...
        if limit_one_per_chemsys:
            ...
            for doc in summ_docs:
                ...
                if doc["energy_above_hull"] < by_chemsys[doc["chemsys"]]["energy_above_hull"]:
                    by_chemsys[doc["chemsys"]] = {k : doc[k] for k in ("material_id","energy_above_hull")}
