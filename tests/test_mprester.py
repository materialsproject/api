...
class TestMPRester:
    ...
    def test_unique_mpids(self, mpr):
        ...
        docs = mpr.summary.search(
            theoretical=False, num_sites=[1, 10], is_stable=True, fields=PROPERTIES, exclude_elements=FBLOCK
        )

        assert len(docs) == len(set([d.material_id for d in docs]))
